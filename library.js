const library = {
       ...
       printPlaylists: function(){
              for(let playlist in this.playlists) {
                     const playlistId =  this.playlists[playlist];     
       ...
       printTracks: function(){
              for(let track in this.tracks) {
                     const trackId = this.tracks[track];
              ...
              for(let playlist in this.playlists) {
                     ...
                     if(playlistId === playlistID){
                            ...
                            for(let i = 0; i < noOfTracks; i++) {
                                   const trackId = this.tracks[trackList[i]];
                                   const trackName = trackId.name;
                                   const artistName = trackId.artist;
                                   const albumName = trackId.album;
                                   console.log(`${trackId}: ${trackName} by ${artistName} (${albumName})`);
       ...
       addTrackToPlaylist:function(trackId, playlistId){
              const existingPlaylist = this.playlists[playlistId].tracks;
              existingPlaylist.push(trackId)
              console.log(existingPlaylist)
       },
       addTrack:function(name, artist, albumm){
...
//                      console.log(`${playlistId}: ${playlistName} ${noOfTracks} tracks`);
//                      for(let i = 0; i < noOfTracks; i++) {
//                             const trackId = library.tracks[trackList[i]];
//                             const trackName = trackId.name;
//                             const artistName = trackId.artist;
//                             const albumName = trackId.album;
//                             console.log(`${trackId}: ${trackName} by ${artistName} (${albumName})`);
...
console.log("======================================");
library.addTrack('LIFETIMES', 'Kety Perry', 'LIFETIMES');
console.log("======================================");
